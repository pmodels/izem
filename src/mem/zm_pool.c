    ...
    if (zm_unlikely(local_pool->num_elements == 0)) {
        ...
        lock_acquire(&global_pool->lock);
        {
            ...
            if (num_taken_blocks != 0) {
                ...
                fast_memcpy(local_pool->blocks, &global_pool->blocks[global_block_i - num_taken_blocks], sizeof(struct block) * num_taken_blocks);
                ...
                if ((global_pool->num_elements & (BLOCKSIZE - 1)) != 0) { // global_pool->num_elements % BLOCKSIZE != 0
                    ...
                    fast_memcpy(&global_pool->blocks[global_block_i - num_taken_blocks], &global_pool->blocks[global_block_i], sizeof(struct block));
                    // fast_memset(&global_pool->blocks[global_block_i - num_taken_blocks + 1], 0, sizeof(struct block) * (global_pool->len_blocks - (global_block_i - num_taken_blocks + 1)));
                } else {
                    // Clear the remaining blocks.
                    // fast_memset(&global_pool->blocks[global_block_i - num_taken_blocks], 0, sizeof(struct block) * (global_pool->len_blocks - (global_block_i - num_taken_blocks)));
                }
            }
...
#else
        if (zm_unlikely(block_num_elements == 0)) {
            block->head = element;
            block->tail = element;
            element->next = NULL;
        } else {
