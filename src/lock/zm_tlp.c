...
#elif (ZM_TLP_HIGH_P == ZM_MCS)
#define zm_tlp_release_high_p(L, local_ctxt) zm_mcs_release(&L->high_p, local_ctxt);
#elif (ZM_TLP_HIGH_P == ZM_HMCS)
#define zm_tlp_release_high_p(L, local_ctxt) zm_hmcs_release(L->high_p);
#endif
...
#elif (ZM_TLP_LOW_P == ZM_MCS)
#define zm_tlp_release_low_p(L, local_ctxt) zm_mcs_release(&L->low_p, local_ctxt);
#elif (ZM_TLP_LOW_P == ZM_HMCS)
#define zm_tlp_release_low_p(L, local_ctxt) zm_hmcs_release(L->low_p)
#endif

#if (ZM_TLP_HIGH_P == ZM_TICKET)
#define zm_tlp_nowaiters_high_p(L, local_ctxt) zm_ticket_nowaiters(&L->high_p, local_ctxt)
#elif (ZM_TLP_HIGH_P == ZM_MCS)
#define zm_tlp_nowaiters_high_p(L, local_ctxt) zm_mcs_nowaiters(&L->high_p, local_ctxt)
#elif (ZM_TLP_HIGH_P == ZM_HMCS)
#define zm_tlp_nowaiters_high_p(L, local_ctxt) zm_hmcs_nowaiters(L->high_p)
#endif
...
/* Release the lock */
int zm_tlp_release(zm_tlp_t *L, zm_mcs_qnode_t *local_ctxt) {
   if (!L->low_p_acq) {
       if (zm_tlp_nowaiters_high_p(L, local_ctxt)) {
           L->go_straight = 0;
           zm_ticket_release(&L->filter);
       }
       zm_tlp_release_high_p(L, local_ctxt);
   } else {
       L->low_p_acq = 0;
       zm_ticket_release(&L->filter);
       zm_tlp_release_low_p(L, local_ctxt);
   }
    return 0;
}
