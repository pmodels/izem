...
static inline int mcs_acquire(struct zm_mcs *) ZM_INLINE_SUFFIX;
static inline int mcs_tryacq(struct zm_mcs *, int *) ZM_INLINE_SUFFIX;
static inline int mcs_release(struct zm_mcs *) ZM_INLINE_SUFFIX;

int zm_imcs_init(zm_mcs_t *);
int zm_imcs_destroy(zm_mcs_t *);

ZM_INLINE_PREFIX static inline int zm_imcs_acquire(zm_mcs_t L) {
    return mcs_acquire((struct zm_mcs*)(void *)L) ;
}

ZM_INLINE_PREFIX static inline int zm_imcs_tryacq(zm_mcs_t L, int *success) {
    return mcs_tryacq((struct zm_mcs*)(void *)L, success) ;
}

ZM_INLINE_PREFIX static inline int zm_imcs_release(zm_mcs_t L) {
...
/* Main routines */
ZM_INLINE_PREFIX static inline int acquire_c(struct zm_mcs *L, zm_mcs_qnode_t* I) {
...
ZM_INLINE_PREFIX static inline int tryacq_c(struct zm_mcs *L, zm_mcs_qnode_t* I, int *success) {
    int acquired  = 0;
    zm_atomic_store(&I->next, ZM_NULL, zm_memord_release);
...
/* Release the lock */
ZM_INLINE_PREFIX static inline int release_c(struct zm_mcs *L, zm_mcs_qnode_t *I) {
...
ZM_INLINE_PREFIX static inline int mcs_acquire(struct zm_mcs *L) {
...
ZM_INLINE_PREFIX static inline int mcs_tryacq(struct zm_mcs *L, int *success) {
    if (zm_unlikely(tid == -1)) {
        check_affinity(L->topo);
        tid = get_hwthread_id(L->topo);
    }
...
ZM_INLINE_PREFIX static inline int mcs_release(struct zm_mcs *L) {
    assert(tid >= 0);
